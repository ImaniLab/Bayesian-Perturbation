...
class TenGene1(Env):
    ...
    def step(self, action: Action) -> Tuple[Tuple[int, int, int, int, int, int, int, int, int, int, np.float64, np.float64, np.float64, np.float64, np.float64, np.float64, np.float64, np.float64, np.float64, np.float64], float, bool, dict]:
        ...
        model_choice = 222 # np.random.choice(np.arange(self.modelNum), 1, p=model_probs) # 222 #np.random.choice(np.arange(self.modelNum), 1, p=model_probs) #222 #np.random.choice(np.arange(self.modelNum), 1, p=model_probs) #222 #242#np.random.choice(np.arange(self.modelNum), 1, p=model_probs)#int((self.agent_pos)[7])#np.random.choice(np.arange(self.modelNum), 1, p=model_probs)

        Cnext = np.atleast_2d(ConnectivitySpace[model_choice, :])
        CnextReshape = np.reshape(Cnext, (number_of_states, number_of_states))

        # calculate x_new based on the the selected model
        ...
        # calculate the M indices and make the new pi
        ...
        #### make sure that the sum of model_probs_new is 1! Important!
        ...
        # set the reward, done signal, and update the states

        done = False
        reward = np.amax(model_probs_new) - np.amax(model_probs) #-10#np.amax(model_probs_new) - np.amax(model_probs)#-1#np.amax(model_probs_new) - np.amax(model_probs)
