...
class TenGene1(Env):
    ...
    def step(self, action: Action) -> Tuple[Tuple[int, int, int, int, int, int, int, int, int, int, np.float64, np.float64, np.float64, np.float64, np.float64, np.float64, np.float64, np.float64, np.float64, np.float64], float, bool, dict]:
        ...
        model_choice = 222#np.random.choice(np.arange(self.modelNum), 1, p=model_probs)#222#np.random.choice(np.arange(self.modelNum), 1, p=model_probs)#int((self.agent_pos)[7])#np.random.choice(np.arange(self.modelNum), 1, p=model_probs)
        ...
        if action == Action.GENE2:
            perturb[2] = 1
        if action == Action.GENE5:
            perturb[5] = 1
        if action == Action.GENE3:
            perturb[3] = 1
        if action == Action.GENE6:
            perturb[6] = 1
